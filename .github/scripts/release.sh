...
if [[ -z "$previous_tag" && -n "${candidate_tags//[[:space:]]/}" ]]; then
  echo "ERROR: Could not determine previous tag for '$version'."
  echo "Release versions must be newer than the existing numeric tags."
  exit 1
fi
...
if [[ -n "$previous_tag" ]]; then
  ...
  compare_link="**Full Changelog**: https://github.com/${repo_slug}/commits/${version}"
fi
...
if [[ "$dryrun" == "true" ]]; then
  ...
  if [[ -n "$previous_tag" ]]; then
    ...
    echo "[DRY RUN] Previous tag: none"
  fi
  echo "[DRY RUN] Changelog link: ${compare_link}"
  exit 0
fi
